...
def load_flipkart_dataset(dataset_path: str) -> ProductIndex:
    ...
    with open(dataset_path, 'r') as f:
        ...
        for product in products:
            ...
            count += 1
            if count % 1000 == 0:  # More frequent progress updates
...
def generate_user_profile(index_data: dict, user_id: int, rng: np.random.Generator) -> dict:
    """Generate a user profile with realistic preferences - optimized with pre-computed weights"""
    
    # Select primary category (using pre-computed weights)
    categories = list(index_data['category_weights'].keys())
    category_probs = list(index_data['category_weights'].values())
    primary_category = rng.choice(categories, p=category_probs)
    ...
    primary_sub_category = rng.choice(sub_categories, p=sub_probs)
    
    # Select preferred brands (1-5 brands)
    brands = list(index_data['brand_weights'].keys())
    brand_weights = list(index_data['brand_weights'].values())
    brand_total = sum(brand_weights)
    brand_probs = [w / brand_total for w in brand_weights] if brand_total > 0 else None
    
    num_brands = rng.integers(1, min(6, len(brands) + 1))
    if brand_probs:
        preferred_brands = list(rng.choice(brands, size=num_brands, replace=False, p=brand_probs))
    else:
        preferred_brands = rng.choice(brands, size=num_brands, replace=False).tolist()
    ...
    brand_loyalty = rng.beta(2, 1)  # Skewed towards higher loyalty
    category_affinity = rng.beta(2, 1)
...
def select_product_for_purchase(index_data: dict,
    ...
    """Select a product for a purchase - minimal fallback"""
    ...
    available = [pid for pid in candidates if pid not in purchased_products]
    
    # If sub-category exhausted, use all products
    if not available:
        all_products = list(index_data['products'].keys())
        available = [pid for pid in all_products if pid not in purchased_products]
...
def generate_user_purchases(index_data: dict, profile: dict, rng: np.random.Generator) -> List[dict]:
    ...
    for i, timestamp in enumerate(timestamps):
        ...
        purchases.append({
            ...
            'quantity': int(quantity),  # Convert numpy int to Python int
            'event_type': 'purchase'
        })
